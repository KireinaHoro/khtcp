...
void async_open(const ip::addr_t src, uint16_t src_port, const ip::addr_t dst,
                ...
                int client_id) {
  if (active) {
    // we're starting a connection
    uint32_t isn = distribution(eng);
    struct conn_key k;
    memcpy(k.src, src, sizeof(ip::addr_t));
    memcpy(k.dst, dst, sizeof(ip::addr_t));
    k.src_port = src_port;
    k.dst_port = dst_port;
    conns.insert({k, {}});
    async_send_segment(
        src, src_port, dst, dst_port, isn, 0, false, false, false, true, false,
        local_window, nullptr, 0,
        [=](int ret) {
          if (!ret) {
            // syn sent successfully, expect a synack
            async_recv_segment(
                dst, dst_port, src, src_port,
                [=](int ret, uint32_t seq_num, uint32_t ack_num, bool ack,
                    bool psh, bool rst, bool syn, bool fin, uint16_t window,
                    const void *data, uint16_t len) {
                  if (!ret) {
                    if (syn && ack && !rst && !fin) {
                      // we get our synack
                      // make sure that the synack does not carry data
                      // check that they've acknowledged our syn
                      if (len || ack_num != isn + 1) {
                        BOOST_LOG_TRIVIAL(warning)
                            << "Malformed connection setup, sending RST";
                        async_send_segment(
                            src, src_port, dst, dst_port, isn + 1, 0, false,
                            false, true, false, false, 0, nullptr, 0,
                            [=](int ret) {
                              if (ret) {
                                BOOST_LOG_TRIVIAL(error)
                                    << "Failed to send RST: errno " << ret;
                              }
                              handler(EINVAL, {});
                            },
                            client_id);
                      ...
                      BOOST_LOG_TRIVIAL(warning)
                          << "Sending ACK for connection establishment";
                      // reply an ack
                      async_send_segment(
                          src, src_port, dst, dst_port, isn + 1, seq_num + 1,
                          true, false, false, false, false, local_window,
                          nullptr, 0,
                          [=](int ret) {
                            if (!ret) {
                              // ack sent, connection now up
                              BOOST_LOG_TRIVIAL(warning)
                                  << "Connection " << k.to_string() << " up";
                              auto &b = conns.at(k);
                              b.pending_receive = boost::asio::mutable_buffer(
                                  new uint8_t[local_window], local_window);
                              b.recv_next = seq_num + 1;
                              b.recv_window = local_window;
                              b.send_next = isn + 1;
                              b.send_unack = ack_num;
                              b.up = true;
                              handler(0, k);
                            } else {
                              BOOST_LOG_TRIVIAL(error)
                                  << "Failed to send ACK: errno " << ret;
                              handler(ret, {});
                            }
                          },
                          client_id);
                    ...
                    BOOST_LOG_TRIVIAL(error)
                        << "Error " << errno << " when waiting for SYNACK";
                  }
                },
                client_id);
          }
