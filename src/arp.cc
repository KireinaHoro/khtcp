...
bool default_handler(int dev_id, uint16_t opcode,
                     core::ptr<const eth::addr> sender_mac,
                     core::ptr<const ip::addr> sender_ip,
                     core::ptr<const eth::addr> target_mac,
                     core::ptr<const ip::addr> target_ip) {
  bool ret = false;
...
void start(int dev_id) {
  boost::asio::post(
      device::get_device_handle(dev_id).read_handlers_strand, [=]() {
        // push to front of queue to ensure that the auto answering does not
        // miss packets and get them passed to the upper layer
        device::get_device_handle(dev_id).read_handlers.push_front(
            wrap_read_handler(default_handler));
      });
}
